...
std::vector<uint8_t, SecureAllocator<uint8_t>> prepare_asymmetric(std::vector<unsigned char, SecureAllocator<unsigned char>>& header, const EncryptRequest& request) {
    ...
    switch (request.algorithm) {
        case CryptoAlgorithms::ECDH_X25519: {
            std::vector<unsigned char, SecureAllocator<unsigned char>> ephemeral_sk(crypto_box_PUBLICKEYBYTES);
            ...
            if (crypto_scalarmult(shared_point.data(), ephemeral_sk.data(), recipient_pk.data()) != 0) {
                unexpected_error("Failed to calculate shared point between the ephemeral secret key and the public key of the recipient");
            }
    ...
    std::to_chars(key_length_str.data(), std::next(key_length_str.data(), key_length_str.size()), key_length);
...
void encrypt(std::vector<unsigned char, SecureAllocator<unsigned char>>& header, const EncryptRequest& request, const std::vector<uint8_t, SecureAllocator<uint8_t>>& key) {
    ...
    std::ranges::transform(encrypted_file_content, encrypted_file_content_char.begin(), [](unsigned char character){
    ...
    file.write(encrypted_file_content_char.data(), static_cast<std::streamsize>(encrypted_file_content_char.size()));
}
