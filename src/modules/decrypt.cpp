...
Result decrypt(const DecryptRequest& request) {
    ...
    if (!std::ranges::equal(byteView, file_signature)) {
        unexpected_error(
            "The passed file has not been encrypted using CFE and cannot be decrypted.");
    }
...
std::vector<uint8_t, SecureAllocator<uint8_t>> prepare_symmetric(
    ...
    const size_t SALT_SIZE = 12;
    const uint8_t TIME_OFFSET = 22;
    const size_t TIME_SIZE = 4;
    const uint8_t MEMORY_OFFSET = 26;
    ...
    auto t_cost_span = std::span(encrypted_file_content.begin() + TIME_OFFSET,
                                 encrypted_file_content.begin() + TIME_OFFSET + TIME_SIZE);
    uint32_t t_cost = 0;
    std::memcpy(&t_cost, t_cost_span.data(), t_cost_span.size());

    auto m_cost_span = std::span(encrypted_file_content.begin() + MEMORY_OFFSET,
                                 encrypted_file_content.begin() + MEMORY_OFFSET + MEMORY_SIZE);
    uint32_t m_cost = 0;
    std::memcpy(&m_cost, m_cost_span.data(), m_cost_span.size());

    auto parallelism = static_cast<uint32_t>(
        static_cast<unsigned char>(encrypted_file_content[PARALLELISM_OFFSET]));
    auto salt = std::span(encrypted_file_content.begin() + SALT_OFFSET,
                          encrypted_file_content.begin() + SALT_OFFSET + SALT_SIZE);
...
std::vector<uint8_t, SecureAllocator<uint8_t>> prepare_asymmetric(
    ...
    std::vector<char, SecureAllocator<char>> sk_path =
        read_password("Please enter the path of the private key used to decrypt: ");
    using SecureString = std::basic_string<char, std::char_traits<char>, SecureAllocator<char>>;
    const SecureString file_name(sk_path.begin(), sk_path.end());
    std::ifstream sk(file_name);

    std::vector<unsigned char, SecureAllocator<unsigned char>> recipient_sk(
        (std::istreambuf_iterator<char>(sk)), std::istreambuf_iterator<char>());

    auto key_length_span =
        std::span(encrypted_file_content.begin() + KEY_LENGTH_OFFSET,
    ...
    switch (algorithm) {
        case CryptoAlgorithms::ECDH_X25519: {
            ...
            if (crypto_scalarmult(shared_point.data(), recipient_sk.data(), key.data()) != 0) {
                unexpected_error(
                    "Failed to calculate shared point between the passed ephemeral public key and "
                    "your secret key");
            }
...
void decrypt(const std::vector<char, SecureAllocator<char>>& encrypted_file_content,
             const std::string& output_path, const CryptoAlgorithms& algorithm,
             const std::vector<unsigned char, SecureAllocator<unsigned char>>& key) {
    const uint8_t IV_OFFSET = 35;
    const size_t IV_SIZE = 12;
    const uint8_t SYMMETRIC_CIPHERTEXT_LENGTH_OFFSET = 43;
    const uint8_t SYMMETRIC_CIPHERTEXT_OFFSET = 47;
    ...
    if (is_asymmetric(algorithm)) {
        auto offset = static_cast<size_t>(ASYMMETRIC_CIPHERTEXT_LENGTH_OFFSET);
        ...
        const std::span<const char> content_span(encrypted_file_content);
        std::memcpy(&ciphertext_len, content_span.data() + SYMMETRIC_CIPHERTEXT_LENGTH_OFFSET,
    ...
    // Retrieve IV
    auto iv_span = std::span(encrypted_file_content.begin() + IV_OFFSET,
                             encrypted_file_content.begin() + IV_OFFSET + IV_SIZE);
    ...
    switch (algorithm) {
        case CryptoAlgorithms::ChaCha20_POLY1305: {
            if (crypto_aead_chacha20poly1305_decrypt(
                    decrypted.data(), &decrypted_text_len, nullptr, ciphertext.data(),
                    ciphertext.size(), header.data(), header.size(), iv.data(), key.data()) != 0) {
                unexpected_error(
